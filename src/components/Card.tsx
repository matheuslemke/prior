...
const Card = ({ id, position, onRemoveCard, onPositionChanged }: CardProps) => {
  return (
    <div
      draggable={true}
      className="absolute flex flex-col overflow-hidden resize-y w-48 h-24 min-h-[55px] border-2 shadow-lg border-neutral-600 rounded-xl"
      style={{
        top: `${position.top}px`,
        left: `${position.left}px`
      }}
      onDragEnd={(evt) => onPositionChanged(id, { left: evt.pageX, top: evt.pageY })}
    >
